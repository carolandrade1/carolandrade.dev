...
export const Button3d = ({href, text}:ButtonComponents) => {
    return (
        <a href={href} className="pushable group block relative w-fit border-none bg-transparent p-0 cursor-pointer outline-offset-4 hover:brightness-110" target="_blank" rel="noopener noreferrer">
            ...
            <span className="absolute top-0 left-0 w-full h-full rounded-xl bg-gradient-to-l from-purple-800 to-purple-800"></span>
            <span className="front block relative py-2.5 px-6 text-white -translate-y-1 group-hover:-translate-y-1.5 group-active:-translate-y-0.5 rounded-xl will-change-transform bg-purple-500">
                {text}
            </span>
        </a>
    )
